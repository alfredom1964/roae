...
module.exports = __webpack_require__(/*! C:\Users\alfredo\roae2\src\main.ts */"zUnb");
...
let AuthInterceptor = class AuthInterceptor {
    ...
    intercept(req, next) {
        return Object(rxjs__WEBPACK_IMPORTED_MODULE_4__["from"])(this.storage.get(TOKEN_KEY))
            .pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_5__["switchMap"])((token) => {
...
let AppComponent = class AppComponent {
    ...
    initializeApp() {
        this.platform.ready().then(() => {
            ...
            // Escribi el guard en favor de esto, puedes ponerlo en todas las rutas que quieras dentro del app-routing.module en la propiedad canActivate
...
let AuthService = class AuthService {
    constructor(http, helper, storage, plt, alertController, router, loadingController, toastController, sms) {
        this.http = http;
        this.helper = helper;
        this.storage = storage;
        this.plt = plt;
    ...
    checkToken() {
        ...
        return this.http.get(`${this.url}/view`).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_7__["map"])((response) => {
            ...
            console.log(e);
            if (e.status === 401 || e.status === 415) {
                // Si no es valida ocultamos la splashscreen, borramos el token y navegamos al login
                this.loading.dismiss();
        ...
        // Dejo esto comentado porque el token que llega no es un JWT
        // this.storage.get(TOKEN_KEY).then(token => {
        //   if (token) {
        //     this.http.get(`${this.url}/view`)
        //     console.log(token)
    ...
    login(credentials) {
        return this.http.post(`${this.url}/login`, credentials).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_7__["tap"])((result) => {
    ...
    getReportsListos() {
        this.loading.present();
        return this.http.get(`${this.url}/mensajes?id=1`).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_7__["tap"])(() => {
            let toast = this.toastController.create({
                message: '  ',
                duration: 3000,
            });
            ...
            return response.reportes_listos;
        }), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_7__["catchError"])((e) => {
            this.loading.dismiss();
            this.showAlert(e.error.msg);
            throw new Error(e);
    ...
    postReporte({ form, coords, reporteId }) {
        ...
        return this.http
            .post(`${this.url}/mensaje`, Object.assign({ // Habria que cambiar este endpoint aqui
            msj, hora: `${hora[0]}${hora[1]}`, fecha: `${fecha[2]}-${fecha[1]}-${fecha[0]}`, fuenteid: '1', tipomensajeid: '1', cuestid: reporteId }, coords))
